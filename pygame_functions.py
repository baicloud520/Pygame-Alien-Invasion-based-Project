...
pygame.init()
pygame.mixer.init()
add_clouds = pygame.sprite.Group()
...
class Clouds(pygame.sprite.Sprite):
    def __init__(self, image, screen_width, screen_height, total):
        pygame.sprite.Sprite.__init__(self)
    ...
    def _make_clouds(self):
        ...
        cloud_y = random.randint(0, self.height)
        self.x = cloud_x
        self.y = cloud_y
        self.rect.x = self.x
        self.rect.y = self.y
    ...
    def _drawme(self):
        add_clouds.draw(screen)


class newLabel(pygame.sprite.Sprite):
    def __init__(self, text, fontSize, xpos, ypos, fontColour, font):
        pygame.sprite.Sprite.__init__(self)
        self.text = text
        ...
        # self.renderText()
        self.pb_font = pygame.font.SysFont("comicsans", self.fontSize)
        self.image = self.pb_font.render(
            self.text, True, (0, 0, 0), False)
        self.rect = self.image.get_rect()
        self.rect.center = [xpos, ypos]
    ...
    #     oldTopLeft = self.rect.center
    #     self.renderText()
    #     self.rect.center = oldTopLeft
    #     if screenRefresh:
    #         updateDisplay()
    ...
    #         (maxWidth, (self.fontSize + 1) * len(textLines) - 15), pygame.SRCALPHA, 32)
...
def makeLabel(text, fontSize, xpos, ypos, fontColour='black', font='Arial'):
    # make a text sprite
    thisText = newLabel(text, fontSize, xpos, ypos, fontColour, font)
    return thisText


def make_cloud(image, screen_width, screen_height, total):
    # Make a cloud sprite.
    number_clouds = int(total)
    for number in range(number_clouds):
        additional_cloud = Clouds(image, screen_width, screen_height, total)
        add_clouds.add(additional_cloud)
...
def updateDisplay():
    ...
    if (keys[pygame.K_ESCAPE]):
        pygame.quit()
        sys.exit()
    # FINALLY I HAVE FOUND. NO NEED FOR BG FILL IN LABELS!!!
